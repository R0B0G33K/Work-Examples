	...
	void discard(Node * a){
		...
		if(p->numChildren == 3){
			...
			if(sib[6] >= 3){
				...
				if(p->numChildren ==1){
					cout << "one child remaining"<<endl;
					thatjusthappened = true;
					that = p->child[2];
				...
				if(p == p->parent->child[2]){
					...
					p->parent->child[0]->child[2]->parent = p->parent->child[0];
					p->parent->value[0] = p->parent->child[0]->child[2]->value[2];
					p->parent->child[0]->numChildren = 3;	
					p->parent->numChildren = 3;
					
					
					
					p->parent->child[0]->value[2] = p->parent->child[0]->child[2]->value[2];
					p->parent->child[0]->value[1] = p->parent->child[0]->child[1]->value[2];
					p->parent->child[0]->value[0] = p->parent->child[0]->child[0]->value[2];
					cout <<p->parent->child[0]->value[0]<<p->parent->child[0]->value[1]<< p->parent->child[0]->value[2]<< endl;
					return discard(p);
				}
				else if(p == p->parent->child[0]){
					p->parent->child[2]->child[1] = p->parent->child[2]->child[0];
