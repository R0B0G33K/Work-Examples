...
void pmerge(int * a, int first1, int last1, int first2, int last2, int my_rank, int p, int * WIN){
		...
		cout << "first1: " << first1<< " last1: "<< last1 << " first2: "<< first2 <<" last2: " << last2<< " totalSize: " << totalSize << endl;
		...
		*/
		...
		for(int i = my_rank; i < numOfShapes; i+=p){
			...
			//cout << "numshapes = " << numOfShapes <<endl;
...
void mergesort(int * a, int first, int last, int my_rank, int p){
	...
	if(first<last){
		...
		pmerge(a, first, middle, middle+1, last, my_rank, p, WIN);
	}
	if(my_rank == 0){
	cout << "first: " << first<< " last: " << last<< " n: " << n << endl;
	cout<< "WIN (incomplete)" <<endl;
	for(int i = 0; i< n; i++){
		cout<< WIN[i] << ", ";
	}
	cout<< endl;
